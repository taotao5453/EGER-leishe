...
class EButton extends egret.DisplayObjectContainer{
    ...
    private assets:egret.SpriteSheet = RES.getRes("game");//名称不一样的话需要修改
    ...
    */
    public constructor(context:any,imgName:string,backFun:Function = null,descStr:string = "",fontSize:number = 30,cartoonType:number = 1,assetsName:string = "game"){
        super();
        this.param.context = context;
        this.init(imgName,backFun,descStr,fontSize,cartoonType,assetsName);
    }

    private init(imgName:string,backFun:Function = null,descStr:string = "",fontSize:number = 30,cartoonType:number = 1,assetsName:string = "game"):void {
